...
/* What the base rate under a drill describes. Named per pool rather than
   described as "the corpus", because the three pools are different populations
   and the number means something different in each. */
var CORPUS_LABEL = {
  positions: 'certified positions',
  drill: 'positions from the drill corpus',
  endgame: 'endgame positions with one passed pawn'
};
...
function renderDrill(e){
  ...
  if(d.mode === 'computed'){
    ...
    return '<div class="drill" id="drill">' +
      ...
      esc(CORPUS_LABEL[d.corpus] || CORPUS_LABEL.positions) + '</p></div>';
  }
...
function wireDrill(e){
  ...
  function question(){
    var it = d.items[i];
    var mover = it.fen.split(' ')[1] === 'w' ? 'White' : 'Black';
    host.innerHTML =
      '<p class="dq"><span class="dn">' + (i + 1) + ' of ' + d.items.length + '</span>' +
        esc(d.question) + '</p>' +
      /* The board is drawn from the mover's side, which is a convention a reader
         has no way of knowing. For the square of the pawn the tempo decides the
         answer outright, so it is said in words on every drill. */
      '<p class="dturn">' + mover + ' to move</p>' +
